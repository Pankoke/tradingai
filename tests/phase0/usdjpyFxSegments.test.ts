...
describe("USDJPY FX watch segments", () => {
  it("derives segments by precedence", () => {
    expect(deriveFxWatchSegment({ ...baseSetup, eventScore: 72 } as Setup)).toBe(FX_WATCH_SEGMENTS.EVENT_RISK_HIGH);
    expect(deriveFxWatchSegment({ ...baseSetup, biasScore: 55 } as Setup)).toBe(FX_WATCH_SEGMENTS.FAILS_BIAS);
