...
const baseSetup: Partial<Setup> = {
  ...
  profile: "SWING",
  timeframeUsed: "1D",
  setupPlaybookId: "gbpusd-swing-v0.1",
  setupDecision: "WATCH",
  setupGrade: "NO_TRADE",
};

describe("GBPUSD FX watch segments", () => {
  it("derives segments by precedence", () => {
    expect(deriveFxWatchSegment({ ...baseSetup, eventScore: 75 } as Setup)).toBe(FX_WATCH_SEGMENTS.EVENT_RISK_HIGH);
