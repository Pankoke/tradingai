...
describe("SetupUnifiedCard mobile progressive disclosure", () => {
  test("list mode keeps driver details collapsed by default with trigger visible", () => {
    render(React.createElement(SetupUnifiedCard, { vm: baseVm, mode: "list", defaultExpanded: true }));

    const trigger = screen.getByTestId("drivers-disclosure-trigger");
    expect(trigger).toBeDefined();
    expect(trigger.getAttribute("aria-expanded")).toBe("false");
    expect(screen.queryByTestId("drivers-detail-content")).toBeNull();
  });

  test("list mode reveals driver details after trigger click", () => {
    render(React.createElement(SetupUnifiedCard, { vm: baseVm, mode: "list", defaultExpanded: true }));
    ...
    expect(screen.getByTestId("drivers-detail-content")).toBeDefined();
  });

  test("sotd mode keeps driver details visible without disclosure trigger", () => {
    render(React.createElement(SetupUnifiedCard, { vm: baseVm, mode: "sotd" }));

    expect(screen.queryByTestId("drivers-disclosure-trigger")).toBeNull();
    expect(screen.getByTestId("drivers-detail-content")).toBeDefined();
  });
});
