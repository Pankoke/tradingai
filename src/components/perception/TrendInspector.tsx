...
export function TrendInspector({ setup, className }: TrendInspectorProps): JSX.Element {
  const t = useT();
  const score = setup.rings?.trendScore ?? 50;
  ...
  return (
    ...
    >
      ...
      <div className="mt-3 space-y-2 text-sm text-slate-300">
        <p>{directionText}</p>
        <p>{t("perception.trend.summary." + bucket)}</p>
      </div>
    </section>
  );
}
