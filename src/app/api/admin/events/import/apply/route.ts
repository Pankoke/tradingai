...
import { applyEventsImport, previewEventsImport } from "@/src/lib/admin/import/eventsImport";
...
export async function POST(request: NextRequest): Promise<Response> {
  ...
  try {
    ...
    const preview = await previewEventsImport(upload.csvText);
    if (preview.summary.errors > 0) {
      return respondFail("BAD_REQUEST", "Fix preview errors before apply", 400, preview.summary);
    }

    const applied = await applyEventsImport({
      csvText: upload.csvText,
      previewHash: upload.previewHash,
    });

    await createAuditRun({
      action: "admin_events_import_apply",
      source: auth.mode,
      ok: true,
      message: "events_import_apply_success",
      meta: buildAuditMeta({
        auth,
