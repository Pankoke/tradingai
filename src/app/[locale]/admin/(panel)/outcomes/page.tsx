...
export default async function OutcomesPage({ params, searchParams }: PageProps) {
  ...
  const showNoTradeType = query.showNoTradeType;
  const includeAllGrades = query.includeAllGrades;
  const includeNoTrade = query.includeNoTrade;
    ...
    <div className="space-y-6">
      <OutcomesIntro
        ...
        sections={[
          ...
          },
          {
            ...
            items: [
              "Totals = DB-Counts (same filter), ignorieren Anzeige-Toggles für Grades/NO_TRADE.",
              "Tabelle kann von Totals abweichen (Limit, Anzeige-Toggles).",
              "Nur Swing 1D; für 1W oder Gesamt-KPIs Artefakt-Seiten nutzen.",
            ],
          },
          {
            heading: "Wann nutzen?",
            items: [
              ...
              "Schneller Health-Check für konkrete Playbooks/Assets.",
              "Nicht als Performance-Gesamtaussage verwenden – dafür Overview/Diagnostics.",
            ],
          },
        ]}
      />
      {query.fromOverview ? (
        <div className="rounded border border-amber-600/50 bg-amber-900/20 p-3 text-xs text-amber-100">
          Hinweis: Du kommst aus der Overview. Explorer zeigt Swing 1D Sample; Label/TF aus Overview werden nicht übernommen.
      ...
      <section className="rounded-lg border border-slate-800 bg-slate-900/60 p-4 shadow-sm space-y-2">
        ...
        <p className="text-xs text-slate-400">
          Totals = DB counts für Swing 1D (Filter: days/asset/playbook). Tabelle unten = Sample (limit 300) + UI-Toggles. Unterschied zu Overview:
          Artefakt-first, 1D/1W, ggf. andere Labels/Aggregate. Hinweis: Totals ignorieren die Anzeige-Toggles für Grades/NO_TRADE (werden nur clientseitig gefiltert).
        </p>
        <p className="text-xs text-slate-400">
